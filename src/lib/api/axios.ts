...
export async function myRequest<T, R>(options: AxiosRequestConfig & { data?: T }): Promise<ResponseModel<R>> {
    try {
        ...
        // 1. Check if it's our custom NetworkError
        ...
        // 3. Handle other, unexpected error types (e.g., if it's a standard Error object)
        ...
        // 4. Fallback for truly unknown error types
        return {
            data: null,
            status: 500,
