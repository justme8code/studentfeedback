...
import {myRequest, ResponseModel} from "@/lib/api/axios";
...
export async function makeAuthRequest<T, R>(
    ...
    // 3. Make the request. The backend will do the real verification.
    const response = await myRequest<T, R>({ ...options, headers });

    // 4. IMPORTANT: Handle the 401 Unauthorized case
